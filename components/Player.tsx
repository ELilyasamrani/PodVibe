...
import { Play, Pause, Download, Share2, Rewind, FastForward, RotateCcw, Volume2, VolumeX, Volume1, Video, Check, Loader2 } from 'lucide-react';
...
const Player: React.FC<PlayerProps> = ({ audioPcm, title, isGenerating, onRegenerateAudio, onTimeUpdate }) => {
  ...
  useEffect(() => {
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        ...
        gradient.addColorStop(0.5, '#e0f2fe'); // Brand 100
  ...
  useEffect(() => {
    ...
    const drawVideoFrame = () => {
      ...
      ctx.fillText("ListenIn", vCanvas.width / 2, 120);
      ...
      for (let i = 0; i < bufferLength; i++) {
        ...
        ctx.fillStyle = '#38bdf8'; // Brand 400
  ...
  const startTeaserRecording = async () => {
    ...
    recorder.onstop = () => {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `ListenIn_Teaser_${title.substring(0, 10)}.webm`;
      a.click();
      
      // Clipboard Text
      const shareText = `Check out this episode "${title}" generated by ListenIn! 🎙️\n\n#AI #Podcast #ListenIn #Tech`;
  ...
  const handleShareLink = () => {
    const text = `Check out this AI-generated podcast about "${title}" on ListenIn!`;
  ...
  return (
    <div className="w-full bg-slate-900 rounded-xl p-6 border border-slate-800 shadow-xl relative overflow-visible">
      ...
      {/* Hidden Canvas for Video Recording (Square 1:1 for LinkedIn) */}
      <canvas 
        ref={videoCanvasRef} 
        width={1080} 
        height={1080} 
      ...
      {recordingState === 'done' && (
        <div className="absolute inset-0 z-50 bg-slate-950/90 backdrop-blur-sm flex flex-col items-center justify-center rounded-xl animate-in fade-in duration-300">
           ...
           <p className="text-slate-400 text-sm mb-1">Text copied to clipboard.</p>
           <p className="text-slate-500 text-xs">Opening LinkedIn...</p>
        </div>
      )}
      ...
      <div className="relative z-10">
              ...
              {isShareMenuOpen && (
                <div className="absolute right-0 top-full mt-2 w-48 bg-slate-800 border border-slate-700 rounded-lg shadow-xl overflow-hidden z-20">
                  ...
                  <button 
                    ...
                    className="w-full text-left px-4 py-3 text-sm text-brand-300 hover:bg-slate-700 hover:text-white flex items-center gap-2 border-t border-slate-700"
                  >
        ...
        {/* Visualizer Area */}
        <div className="h-32 w-full bg-slate-950/50 rounded-xl mb-6 overflow-hidden relative border border-slate-800/50 flex items-center justify-center">
